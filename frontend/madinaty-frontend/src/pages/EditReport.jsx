...
const EditReport = () => {
  ...
  const reportId = location.state?.id; // 👈 comes from navigate('/edit', { state: { id: report.id } })
  ...
  useEffect(() => {
    ...
    if (reportToEdit) {
      ...
      setAddress(reportToEdit.location);
      setImage(reportToEdit.image);
    }
  }, [reportId]);
  ...
  return (
    <div className="w-full h-screen flex lg:p-20 flex-col justify-between overflow-y-auto direction-rtl p-4 gap-10">
      ...
      <div className="flex flex-col">
        <h2 className="text-[32px] opacity-70">تعديل البلاغ</h2>
        ...
        <label className="bg-gray-300 w-[136px] mt-2 border-[0.3px] border-gray-400 h-[50px] rounded-[15px] flex justify-center items-center cursor-pointer">
          <p className="opacity-50 text-xl">تغيير الصورة</p>
          <input
            type="file"
            accept="image/*"
            className="hidden"
